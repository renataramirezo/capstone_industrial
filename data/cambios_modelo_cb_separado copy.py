...
def main():
    try:
        ...
        for t in T:
            sol_bases_faenas[t] = []
            sol_hectareas_cosechadas[t] = T       
        
        for t in T:
            for i in G.nodes():
                for k in K:
                    if (i, k, t) in f and f[i, k, t].X > 0.5:
                        sol_bases_faenas[t].append(i)
                    for j in G.nodes():
                        if (i, j, k, t) in x and x[i, j, k, t].X > 0.5:
                            sol_hectareas_cosechadas[t].append(j)
    ...
    try:
        ...
        with open(archivo_txt, 'w', encoding='utf-8') as txt_file:
            ...
            for (i, j, k, t), cantidad in datos['variables']['w'].items():
                if cantidad > 0:
                    txt_file.write(f"    - Faena {i} → hectárea {j}, máquina {k}, período {t}, cosecha: {cantidad:.2f} m3\n")
